...
//Animación del coche
float angRot = 0.0;
float movKit = 0.0, movKit2 = 0.0;
float rotTires = 0.0;
bool g_fanimacion = false;
float rotLlanta = 0.0, rotLlanta2 = 0.0;
...
void display ( void )   // Creamos la funcion donde se dibuja
{
		...
		glPushMatrix();		
			...
			glPushMatrix();
				...
				glPushMatrix();
					...
					glPushMatrix(); //Llanta derecha - f
						glTranslatef(-6.0, 0.0f, 7.5);
						glRotatef(rotLlanta, 1.0, 0.0, 0.0);
						glRotatef(rotLlanta2, 0.0, 0.0, 1.0);
						llanta.GLrender(NULL, _SHADED, 1.0);
					glPopMatrix();

					glPushMatrix();//Llanta izquierda - f
						glTranslatef(6.0, 0.0f, 7.5);
						glRotatef(rotLlanta, 1.0, 0.0, 0.0);
					...
					glPushMatrix(); //Lanta derecha - t
						glTranslatef(-6.0, 0.0f, -9.5);
						glRotatef(rotLlanta, 1.0, 00.0, 0.0);
						
						glRotatef(rotLlanta2, 0.0, 0.0, 1.0);
						llanta.GLrender(NULL, _SHADED, 1.0);
					glPopMatrix();

					glPushMatrix();//Llanta izquierda - t
						glTranslatef(6.0, 0.0f, -9.5);
						glRotatef(rotLlanta, 1.0, 0.0, 0.0);
						glRotatef(180, 0.0, 1.0, 0.0);
						glRotatef(rotLlanta2, 0.0, 0.0, 1.0);
	...
	glDisable(GL_TEXTURE_2D);
		glDisable(GL_LIGHTING);
			...
			pintaTexto(-0.25,0.23,-0.25,(void *)font,"Practica 10");
...
//-------------------------------------------
	...
	if (g_fanimacion)
		...
		case 7: //
			...
			break;
		case 8: //regresa a origen

			movKit += 0.8;
			rotLlanta += 8.0;
			if (movKit >= 0) {
				ciclo = 0;
				g_fanimacion = false; //volvemos fanimacion falso para que deje de animarse y poder volver a presionar "barra espaciadora"
			}
